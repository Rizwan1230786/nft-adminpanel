        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Nft Items</h4>
                </div>
                <div class="table-responsive width-95-per mx-auto">
                    <div class="dt-buttons float-end" style="margin-left: 20px; margin-top: 14px;">
                        <button class="dt-button add-new btn btn-success waves-effect waves-float waves-light"
                            ...
                            onclick="window.location.href='/admin/item-create'">
                            <span>Add New</span>
                        </button>
                    </div>
                    <table class="table datatable table-bordered">
                        ...
                        <tbody>
                            ...
                            @foreach ($user as $data)
                                <tr>
                                    <td class="count"></td>
                                    <td><img src="{{ asset('images/items/' . $data->image) }}" width="50" height="50"
                                            style="border-radius:10px;" alt=""></td>
                                    <td>{{ Str::limit($data->name, 20) }}</td>
                                    <td title="{{ $data->price }}">{{ $data->price }}</td>
                                    <td title="{{ $data->size }}">{{ $data->size }}</td>
                                    <td>{{ $data->no_of_copies }}</td>
                                    @php
                                        $user_name = Customer::where('id', $data->user_id)->first();
                                    @endphp
                                    <td>{{ $user_name->firstname ?? 'Admin' }}</td>
                                    <td>
                                    ...
                                    <td class="d-flex">
                                        <div>
                                            <a class="dropdown-item1" href="/admin/edit/item/{{ $data->id }}">
                                        ...
                                        <div>
                                            <button class="btn btn-flat btn-sm remove-user dropdown-item1"
                                                data-id="{{ $data->id }}"
                                                data-action="{{ url('/item/delete', $data->id) }}"
